...
Article.prototype.init = function () {
  ...
  this.xhttp.onload = function () {
    ...
    for (let d in self.data) {
      self.list.innerHTML += `<li id="a-${d}">${self.data[d]["name"]}</li>`;
      self.citation.innerHTML += `<p>${self.data[d]["source"]}</p>`;
    }

    const article_button = document.querySelectorAll("#articles-list li");
    article_button.forEach((el, index) => {
      if (el != null || el != undefined) {
        el.addEventListener("click", () => self.open(self.data[index]));
      }
    });
  };
  this.xhttp.open(
    "GET",
    "https://api.jsonbin.io/v3/b/610b7c16f098011544ab8a04",
    true
  );
  this.xhttp.send();
};
