...
Gallery.prototype.fetchData = function () {
  ...
  xhttp.onload = function () {
    ...
    for (let d in self.data) {
      self.parent.children[0].innerHTML += `<img draggable="false" src="assets/images/${self.data[d]["image"]}" alt="">`;
      self.list.innerHTML += `<div></div>`;
      linkimg.innerHTML += `<p>${self.data[d]["linkImg"]}</p>`;
    }
...
Gallery.prototype.up = function (e) {
  ...
  if (this.pos_up != 0) {
    if (this.pos_down < this.pos_up && this.num > 1) {
      ...
      this.content.children[1].innerHTML =
        this.data[this.num - 1]["description"];
      this.clearList();
